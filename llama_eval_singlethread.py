...
def generate_text_from_image(model, processor, image, prompt_text, temperature, top_p, device):
    try:
        combined_prompt = f"""<|begin_of_text|><|start_header_id|>system<|end_header_id|>你是一個旅遊專家，能十分準確的分析圖片中的景點。所有對話請用繁體中文進行。<|end_of_text|>
    ...
    for image_path in image_files:
        try:
            ...
            if image:
                ...
                # Location: 921地震教育園區
                location_name = str(image_path).split('/folder_')[1][2:].split('-')[0]
